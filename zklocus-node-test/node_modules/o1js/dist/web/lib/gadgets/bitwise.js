...
import { MAX_BITS, assert, divideWithRemainder, toVar, exists, bitSlice, } from './common.js';
...
function rotate64(field, bits, direction = 'left') {
    ...
    if (field.isConstant()) {
        assert(field.toBigInt() < 1n << BigInt(MAX_BITS), `rotation: expected field to be at most 64 bits, got ${field.toBigInt()}`);
